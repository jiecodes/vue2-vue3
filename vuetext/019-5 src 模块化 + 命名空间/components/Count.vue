<template>
  <div class="count" >
    ...
    <!-- vuex 模块化 -->
    <!-- <div v-model="number">数字和{{countAbout.sum}}</div> -->
    <!-- <div v-model="number">数字10倍和{{$store.state.sum * 10}}</div> -->
    <!-- getters -->
    <!-- <div v-model="number">数字10倍和{{tenDouble}}</div> -->
    <!-- <div>{{countAbout.school}},{{countAbout.subject}}</div>  -->
    <!-- <div>展示Persons.vue 有几个人 {{this.$store.state.personsList.length}}</div> -->
    <!-- mapState 计算属性 -->
    <!-- <div>展示Persons.vue 有几个人 {{personAbout.personsList.length}}</div> -->

    <!-- vuex 模块化 不写 countAbout. store.js中写 namespaced: -->
    <div v-model="number">数字和{{sum}}</div>
    <!-- <div v-model="number">数字10倍和{{$store.state.sum * 10}}</div> -->
    <!-- getters -->
    <div v-model="number">数字10倍和{{tenDouble}}</div>
...
export default {
  ...
  computed: {
    ...
    // store 模块化  获取state中的数据
    // ...mapState(['countAbout','personAbout',]),
    // store 模块化 获取state中的countAbout里面的数据 必须在store.js每个配置项中写 namespaced:true,  目的:{{countAbout.sum}}不写countAbout 
    ...
    ...mapGetters('countAbout',['tenDouble'])
  },
  methods: {
    // increment() {
    ...
    // store 模块化 里面的方法必须告诉是哪个对象里面的      错误:unknown mutation type: increment1
    ...mapMutations('countAbout',{increment: 'increment1',}),
    ...
    ...mapActions('countAbout',{decrement:'decrement1', incrementOdd:'incrementOdd1', incrementWait:'incrementWait1'})
  },
  mounted() {
    // 组件实例对象 vc 有了 $store
